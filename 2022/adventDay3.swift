...
var priorityItems = [String.Element]()

for bag in bags {
    let compartments = Array(bag)
    let size = compartments.count
    let leftCompartment = compartments.prefix(size / 2)
    let rightCompartment = compartments.suffix(size / 2)
