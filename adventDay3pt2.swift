...
var bags = input.components(separatedBy: "\n")

var priorityItems = [Character]()

for i in stride(from: 0, to: bags.count, by: 3) {
    let compartment1 = Array(bags[i]) 
    let compartment2 = Array(bags[i+1]) 
    let compartment3 = Array(bags[i+2]) 

    for item in compartment1 {
        if compartment2.contains(item) && compartment3.contains(item) {
...
for item: String.Element in priorityItems { 
    if item.isLowercase {
        score += Int(item.asciiValue!) - 96
    } else if item.isUppercase {
        score += Int(item.asciiValue!) - 38
    }
}
